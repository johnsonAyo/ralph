...
import { ExtractionConfidence, ReportResult, ReportVerdictCode, } from "@ralph/shared";
...
test("returns the primary result when the primary analyser succeeds", async () => {
    const service = new FallbackRalphAnalyserService(fakeAnalyser(primaryResult), fakeAnalyser(fallbackResult));
...
test("returns the fallback result when the primary analyser throws", async () => {
    const service = new FallbackRalphAnalyserService(fakeAnalyser(new AppError("OpenAI is down", 502, "OPENAI_ANALYSIS_FAILED")), fakeAnalyser(fallbackResult));
    const result = await service.analyse(stubContext);
    assert.equal(result.summary, "fallback-output");
});
test("throws the fallback analyser error when both providers fail", async () => {
    const service = new FallbackRalphAnalyserService(fakeAnalyser(new AppError("OpenAI is down", 502, "OPENAI_ANALYSIS_FAILED")), fakeAnalyser(new AppError("Ollama is down", 502, "OLLAMA_ANALYSIS_FAILED")));
    await assert.rejects(() => service.analyse(stubContext), (err: unknown) => {
