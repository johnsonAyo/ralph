...
import { AuctionPlatformCode, ExtractionConfidence, ReportResult, ReportVerdictCode, } from "@ralph/shared";
...
test("OllamaRalphAnalyserService posts the structured-output request to /api/chat and parses the response", async () => {
    const mock = installFetchMock(async ({ url, init }) => {
        ...
        if (url === "http://localhost:11434/api/chat") {
            ...
            return new Response(JSON.stringify({ message: { content: JSON.stringify(reportJson) } }), {
                status: 200,
    ...
    try {
        ...
        assert.equal((chatCall!.init.headers as Record<string, string>)["Content-Type"], "application/json");
    }
    finally {
        mock.restore();
    }
