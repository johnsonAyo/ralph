...
import { STRIPE_CHECKOUT_CANCEL_PATH, STRIPE_CHECKOUT_SUCCESS_PATH, } from "./payments.constants";
@Controller("payments")
export class PaymentsController {
    constructor(
    @Inject(STRIPE_CLIENT)
    private readonly stripe: Stripe, private readonly config: ConfigService, private readonly webhookService: StripeWebhookService) { }
    ...
    @Body()
    body: CreateCheckoutRequest): Promise<{
        url: string;
    }> {
        const { tier } = body;
    ...
    private async buildLineItems(tier: StripePriceLookupKey) {
        ...
        if (!price) {
            throw new AppError(`No Stripe price found for lookup key "${tier}". Create it in the Stripe dashboard.`, 500, "STRIPE_PRICE_NOT_FOUND");
        }
        return {
