...
export class StripeWebhookService {
  ...
  private async handleCheckoutCompleted(session: Stripe.Checkout.Session): Promise<void> {
    ...
    if (!metadata?.userId || !metadata?.tier) {
      throw new AppError(
        "Stripe session missing userId or tier metadata.",
        400,
    ...
    if (!credits) {
      throw new AppError(
        `Unknown Stripe tier in metadata: ${metadata.tier}`,
        400,
        "STRIPE_UNKNOWN_TIER",
      );
    }
    ...
    if (error) {
      throw new AppError(
        "Failed to top-up credits after Stripe payment.",
        500,
        "CREDITS_TOPUP_FAILED",
        error.message,
      );
    }
  }
