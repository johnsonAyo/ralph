  ...
  providers: [
    ...
    OllamaRalphAnalyserService,
    {
      ...
      useFactory: (
        ...
        openAiAnalyser: RalphAnalyserPort,
        ollamaAnalyser: RalphAnalyserPort,
      ): RalphAnalyserPort => {
        ...
        if (!hasOpenAi && !hasOllama) {
          throw new Error(
            "Ralph analyser requires at least one LLM provider. Set OPENAI_API_KEY or OLLAMA_URL in apps/api/.env.",
          );
        }
