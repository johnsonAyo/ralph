...
import {
  RALPH_ANALYSIS_MODEL_FALLBACK,
  RALPH_ANALYSIS_RESPONSE_FORMAT,
  ...
  async analyse(input: RalphAnalysisContext): Promise<ReportResult> {
    ...
    try {
      ...
      if (!response.output_text) {
        throw new AppError(
          "OpenAI returned an empty Ralph analysis response.",
          502,
      ...
      if (error instanceof SyntaxError) {
        throw new AppError(
          "OpenAI returned malformed JSON for the Ralph report.",
          502,
          "OPENAI_INVALID_JSON",
          error.message,
        );
      ...
      throw new AppError(
        "Unable to generate the Ralph analysis report.",
        502,
