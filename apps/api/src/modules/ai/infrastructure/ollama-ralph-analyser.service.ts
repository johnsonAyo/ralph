...
import { RALPH_ANALYSIS_OLLAMA_MODEL_FALLBACK, RALPH_ANALYSIS_TIMEOUT_MS, } from "./ralph-analysis.constants";
...
export class OllamaRalphAnalyserService implements RalphAnalyserPort {
    ...
    async analyse(input: RalphAnalysisContext): Promise<ReportResult> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new AppError(`Ollama returned ${response.status} for the Ralph analysis.`, 502, "OLLAMA_ANALYSIS_FAILED", bodyText.slice(0, 500));
            }
            const payload = (await response.json()) as OllamaChatResponse;
            const text = payload.message?.content;
            if (!text) {
                throw new AppError("Ollama returned an empty Ralph analysis response.", 502, "OLLAMA_EMPTY_RESPONSE");
            }
            let parsed: unknown;
            try {
                ...
                throw new AppError("Ollama returned malformed JSON for the Ralph report.", 502, "OLLAMA_INVALID_JSON", parseError instanceof Error ? parseError.message : String(parseError));
            }
            try {
                return reportResultSchema.parse(parsed);
            }
            catch (schemaError) {
                const details = schemaError instanceof ZodError
                    ? schemaError.issues
                    : schemaError instanceof Error
                        ? schemaError.message
                        : String(schemaError);
                throw new AppError("Ollama returned a payload that did not match the Ralph report schema.", 502, "OLLAMA_INVALID_SCHEMA", details);
            }
        }
        catch (error) {
            ...
            throw new AppError(isAbort
                ? `Ollama timed out after ${RALPH_ANALYSIS_TIMEOUT_MS}ms.`
                : "Unable to generate the Ralph analysis report from Ollama.", 502, code, message);
        }
        finally {
    ...
    private async fetchImagesAsBase64(urls: string[]): Promise<string[]> {
        ...
        if (dropped.length > 0) {
            this.logger.warn(`Ollama analyser dropped ${dropped.length}/${urls.length} listing photos: ${dropped.join(", ")}. Damage assessment may be weaker as a result.`);
        }
