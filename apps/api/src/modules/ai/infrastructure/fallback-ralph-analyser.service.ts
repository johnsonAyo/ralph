...
export class FallbackRalphAnalyserService implements RalphAnalyserPort {
  ...
  constructor(
    private readonly primary: RalphAnalyserPort,
    private readonly secondary: RalphAnalyserPort,
  ) {}
  ...
  async analyse(input: RalphAnalysisContext): Promise<ReportResult> {
    try {
      ...
      const primaryCode =
        primaryError instanceof AppError ? primaryError.code : "UNKNOWN_PRIMARY";
      this.logger.warn(
        `Primary Ralph analyser failed (${primaryCode}: ${primaryMessage}). Attempting fallback provider.`,
      );
