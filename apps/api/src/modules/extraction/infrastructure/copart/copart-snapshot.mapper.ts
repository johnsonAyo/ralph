...
import { CopartDynamicFieldCode, CopartFieldCode, COPART_REQUIRED_FIELDS, } from "@/modules/extraction/domain/constants/copart.constants";
import { JsonRecord } from "@/modules/extraction/domain/types/json-record.type";
import { confidenceFromMissingFields, findLineStarting, firstMatch, lotNumberFromCopartUrl, missingRequiredFields, parseMoney, parseInteger, recordValue, toNumber, toStringValue, valueAfter, } from "@/modules/extraction/infrastructure/shared/parsing.utils";
...
export function buildCopartListingSnapshot(input: {
    ...
    const offers = recordValue<JsonRecord>(product, "offers") ?? recordValue<JsonRecord>(vehicle, "offers");
    ...
    const snapshot = {
        ...
        lotNumber: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.LotNumber)) ??
        ...
        title: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.LotDescription)) ??
            toStringValue(recordValue(vehicle, "name")) ??
            toStringValue(recordValue(product, "name")) ??
            firstMatch(lines, /20\d{2}\s+[A-Z]/),
        year: toNumber(recordValue(cachedLotDetails, CopartFieldCode.Year)) ??
            toNumber(recordValue(vehicle, "vehicleModelDate")),
        make: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.Make)) ??
            toStringValue(recordValue(brand, "name")),
        model: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.Model)) ??
            toStringValue(recordValue(vehicle, "model")),
        mileage: toNumber(recordValue(cachedLotDetails, CopartFieldCode.Mileage)) ??
        ...
        currentBid: toNumber(recordValue(dynamicLotDetails, CopartDynamicFieldCode.CurrentBid)) ??
        ...
        buyItNowPrice: toNumber(recordValue(dynamicLotDetails, CopartDynamicFieldCode.BuyTodayBid)) ??
        ...
        location: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.Location)) ??
            valueAfter(lines, "Sale name:") ??
            valueAfter(lines, "Location:"),
        category: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.Category)) ??
            valueAfter(lines, "Category:"),
        primaryDamage: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.PrimaryDamage)) ??
            valueAfter(lines, "Primary damage:"),
        secondaryDamage: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.SecondaryDamage)) ??
            valueAfter(lines, "Secondary damage:"),
        runCondition: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.RunCondition)) ??
            valueAfter(lines, "Run condition:"),
        hasKeys: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.HasKeys))?.toLowerCase() ===
            "yes" || lines.some((line) => line.toLowerCase() === "has keys"),
        v5Status: toStringValue(recordValue(cachedLotDetails, CopartFieldCode.V5Status)) ??
            valueAfter(lines, "Has key:"),
        estimatedRetailValue: toNumber(recordValue(cachedLotDetails, CopartFieldCode.EstimatedRetailValue)) ??
