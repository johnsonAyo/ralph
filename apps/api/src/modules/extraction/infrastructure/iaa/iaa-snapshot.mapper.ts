...
import { IAA_REQUIRED_FIELDS, IaaLabel, IaaValue, IAA_PRICE_LINE_PATTERN, IAA_SALE_DATE_LINE_PATTERN, UK_POSTCODE_LINE_PATTERN, } from "@/modules/extraction/domain/constants/iaa.constants";
import { confidenceFromMissingFields, firstMatch, lastNumericPathSegment, missingRequiredFields, parseInteger, parseMoney, stripInfoPrefix, uniqueImages, valueAfter, } from "@/modules/extraction/infrastructure/shared/parsing.utils";
...
export function buildIaaListingSnapshot(input: {
    ...
    const snapshot = {
        ...
        primaryDamage: valueAfter(input.lines, IaaLabel.Description) ?? valueAfter(input.lines, IaaLabel.Category),
