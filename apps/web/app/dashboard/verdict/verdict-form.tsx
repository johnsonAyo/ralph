...
import type { VehicleVerdictRequest } from "@ralph/shared";

interface VerdictFormProps {
  onSubmit: (data: VehicleVerdictRequest) => Promise<void>;
  isLoading: boolean;
  error?: string;
}

export function VerdictForm({ onSubmit, isLoading, error }: VerdictFormProps) {
  ...
  const isReady = registration.length >= 2 && Number(totalBudget.replace(/,/g, "")) > 0;
      ...
      <div className="flex flex-col gap-6">
        <label className="flex flex-col gap-2">
          ...
          <Input
            ...
            required
          />
          <span className={HINT_TEXT}>Enter the UK registration plate.</span>
        </label>
