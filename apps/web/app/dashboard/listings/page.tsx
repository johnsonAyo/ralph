    ...
    <div className="dashboard-page">
      <section className="dash-section">
        <header className="dash-section-head" style={{ marginBottom: "2rem" }}>
          <div>
            <p className="dash-kicker">Find Cars</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
          {SUPPORTED_SITES.map((site) => (
            <a 
              ...
              className="dash-card"
              style={{ display: 'flex', flexDirection: 'column', gap: '16px', textDecoration: 'none', color: 'inherit', transition: 'all 0.2s', border: '1px solid var(--line)' }}
            >
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: 'auto' }}>
                {site.tags.map((tag) => (
                  <span key={tag} style={{ fontSize: '0.75rem', padding: '4px 10px', background: 'var(--surface-dim)', borderRadius: '12px', fontWeight: 600 }}>
                    {tag}
                  </span>
                ))}
              </div>
