...
interface UseSignInLinkResendOptions {
    email: string | undefined;
    emailRedirectTo: string;
    cooldownMs?: number;
}
export function useSignInLinkResend({ email, emailRedirectTo, cooldownMs = DEFAULT_COOLDOWN_MS, }: UseSignInLinkResendOptions) {
    ...
    const submit = useCallback(async (event: FormEvent) => {
        ...
        try {
            ...
            setResendMessage("Sign-in link sent. Check your inbox.");
        }
        catch (err) {
            setResendMessage(err instanceof Error ? err.message : "Failed to send sign-in link.");
        }
        finally {
